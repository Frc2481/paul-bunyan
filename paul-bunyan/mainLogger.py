...
if not os.system("mount | grep TOM") == 0:
	os.system("mount /dev/sda1 /media/TOM")
	time.sleep(.5)
...
loopsPerFlush = 50 #Assume we log at 5 Hz, with a flush every 10 seconds.  5 loops/second * 10 seconds/flush = 50 loops/flush
...
existingLogs = glob.glob("/media/TOM/logFile-*.csv")
eLNumMax = -1
for el in existingLogs:
	el = int(el[19:-4])
	if el > eLNumMax:
		eLNumMax = el
eLNumMax += 1
logFile = open("/media/TOM/logFile-" + str(eLNumMax) + ".csv", 'w', bufsize)
	...
	if len(sdkeysToLog) > 0 or len(logkeysToLog) > 0:
		for key in sdkeysToLog:
			...
			if subkey == "GAMEOVER":
				print "HI"
				print val
			if subkey == "GAMEOVER" and val == "True":
				matchOver = True
